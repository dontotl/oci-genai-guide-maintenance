...
| 조회 항목 | 실행 명령 | 상태 | 관찰 |
|---|---|---|---|
| 구독 리전 조회 | `oci iam region-subscription list --all --tenancy-id <tenancy_ocid> --output table` | 실패 (`timeout 15s`) | `identity.ap-seoul-1.oci.oraclecloud.com` GET 요청이 반복되었지만 응답 상태 미수신 |
| GPU shape 조회 | `oci --region us-ashburn-1 compute shape list --all -c <tenancy_ocid> ...` | 실패 (`timeout 15s`) | `iaas.us-ashburn-1.oraclecloud.com` GET 요청이 반복되었지만 응답 상태 미수신 |
| 보조 연결 확인 | `oci --region ap-seoul-1 os ns get --output table` | 실패 (`timeout 15s`) | `objectstorage.ap-seoul-1.oraclecloud.com` GET 요청이 반복되었지만 응답 상태 미수신 |
...
| `region-subscription list` | Identity endpoint 요청 반복, HTTP status/본문 미수신 | Oracle 리전 문서 기준 표로 대체 |
| `compute shape list` | IaaS endpoint 요청 반복, HTTP status/본문 미수신 | Compute/Data Science shape 문서 기준 해석표로 대체 |
| `os ns get` | Object Storage endpoint 요청 반복, HTTP status/본문 미수신 | 인증 성공/실패 판정보다 앞단의 reachability 또는 응답 지연 문제로 취급 |
...
| `xAI Grok 4.20` | reasoning + external API tool use | 256k | 예 | 불가 | 긴 툴체인, browser/search 스타일 워크플로 |
...
|---|---|---|---|---|
| `cohere.embed-v4.0` | 임베딩 | 예 | 불가 | 텍스트 + 이미지 임베딩, configurable dimensions |
| `Cohere Rerank 4.0 Fast` | 재정렬 | 예 | 불가 | 빠른 검색 후 재정렬 |
| `Cohere Rerank 4.0 Pro` | 재정렬 | 예 | 불가 | 높은 품질의 재정렬 |
| `Cohere Rerank 3.5` | 재정렬 | 아니오 | 불가 | 기존 dedicated-only 재정렬 기준선 |
...
| 모델 | 유형 | 온디맨드 | 파인튜닝 | 핵심 강점 |
|---|---|---|---|---|
| `xAI Voice (Text to Speech)` | 음성 합성 | 예 | 불가 | 텍스트 입력을 음성으로 변환 |
...
| `Cohere Rerank 4.0 Fast` | `RERANK_COHERE x1` | 불가 | Oracle release notes + 모델 페이지 기준 dedicated 가능 |
| `Cohere Rerank 4.0 Pro` | `RERANK_COHERE x1` | 불가 | Oracle release notes + 모델 페이지 기준 dedicated 가능 |
| `Cohere Rerank 3.5` | `RERANK_COHERE x1` | 불가 | dedicated only |
...
| `Cohere Rerank 4.0 Pro/Fast` | 검색 후 재정렬 품질을 dedicated로 고정 운영 가능 | dedicated regional shape 표는 개별 모델 페이지와 `Models by Region`으로 교차 확인 |
...
| `Qwen/Qwen3.6-35B-A3B` | `H100_X2` | `2026-05-11` release notes 추가 |
| `Qwen/Qwen3.5-9B` | `H100_X1` | `2026-05-11` release notes 추가 |
...
| `google/gemma-4-31B-it` | `H100_X2` | `2026-05-11` release notes 추가 |
...
| `Cohere Rerank 4.0 Pro/Fast` | 불가 | rerank 모델, hosting 전용 |
| `Cohere Rerank 3.5` | 불가 | dedicated only |
...
| `xAI Voice (Text to Speech)` | 불가 | 음성 합성, hosting/fine-tuning 표와 별도 |
...
| `cohere.embed-v4.0` | 텍스트 + 이미지 임베딩 + configurable dimensions |
| `Cohere Rerank 4.0 Fast` | 빠른 검색 후 재정렬 |
| `Cohere Rerank 4.0 Pro` | 더 높은 품질의 검색 후 재정렬 |
| `Cohere Rerank 3.5` | 기존 dedicated 재정렬 기준선 |
...
| `xAI Grok 4.20` | external API tool use가 강조된 reasoning |
| `xAI Grok Code Fast 1` | agentic coding, tool-use 중심 |
| `xAI Voice (Text to Speech)` | 텍스트를 음성으로 바꾸는 TTS 워크로드 |
...
- 재정렬 품질 우선: `Cohere Rerank 4.0 Pro`
- 재정렬 속도 우선: `Cohere Rerank 4.0 Fast`
- 코딩 에이전트: `xAI Grok Code Fast 1` 또는 `openai.gpt-oss-20b`
- TTS 시작점: `xAI Voice (Text to Speech)`
...
### 15-3. 이번 기준에서 새로 눈에 띄는 추가 항목

| 구분 | 추가 항목 | 메모 |
|---|---|---|
| 신규 모델 | `xAI Voice (Text to Speech)` | `2026-05-15` release notes |
| 신규 모델 | `Cohere Rerank 4.0 Pro`, `Cohere Rerank 4.0 Fast` | `2026-05-09` release notes |
| 기능 확장 | `Cohere Embed 4` | configurable dimensions, mixed text+image payload |
| import 호환 모델 추가 | `Qwen/Qwen3.6-35B-A3B`, `Qwen/Qwen3.5-9B`, `google/gemma-4-31B-it` | `2026-05-11` release notes |
...
| 조회 항목 | 성공/실패 | 최종 처리 |
|---|---|---|
| `oci iam region-subscription list` | 실패 | HTTP status 미수신, Oracle 공식 리전 문서 기준으로 대체 |
| `oci compute shape list` | 실패 | HTTP status 미수신, Compute/Data Science shape 문서 기준으로 대체 |
| `oci os ns get` | 실패 | HTTP status 미수신, endpoint reachability 확인용 보조 근거로만 사용 |
...
- imported 모델 release notes: `Compatible Imported Models in OCI Generative AI`
