...
리전별 스냅샷을 화면에서 필터링하려면 GitHub Pages GUI `https://dontotl.github.io/oci-genai-guide-maintenance/catalog.html` 또는 이 저장소의 [catalog.html](../catalog.html)을 함께 보시면 됩니다. Catalog의 컬럼별 source badge, query status, retry 취합 기준은 [catalog-notes.md](../catalog-notes.md)를 참고하시면 됩니다. 네트워크 보안 설계는 [private endpoint 별첨](../appendix/private-endpoint-architecture.md)을 참고하시면 됩니다. 단, private endpoint는 미지원 리전에 모델이나 GPU capacity를 생성하는 기능이 아닙니다. private endpoint는 지원 리전에 있는 Generative AI endpoint를 private network로 접근하는 패턴입니다.
...
| 공개 catalog 상태 반영 | AI catalog 공개 JSON은 43개 리전 행을 포함합니다. Catalog UI는 컬럼별 source badge, 서비스별 query status, `query_attempts`를 함께 표시합니다. API별 success/failed/timeout 상태는 공식 지원과 capacity 보장으로 단정하지 않습니다. |
...
| AI catalog 공개 JSON | 43개 리전 행 생성 | 공개 JSON은 UI 필터용 스냅샷입니다. Catalog UI는 `CLI query`, `Oracle docs reference` source badge와 `Query Details`를 함께 표시합니다. GenAI 모델 조회는 일부 리전에서 실패/타임아웃이 있으므로, 표의 상태 라벨과 Oracle 공식 문서를 함께 확인해야 합니다. |
| Catalog query attempts | 공개 JSON에 `query_attempts` 포함 | 기본 `fast` profile은 1회 빠른 수집을 사용하고, `balanced`/`deep` profile에서 재시도와 best-result 취합을 사용합니다. `timeout`/`failed`는 미지원이 아니라 조회 불완전이며, 재조회하면 결과가 달라질 수 있습니다. |
