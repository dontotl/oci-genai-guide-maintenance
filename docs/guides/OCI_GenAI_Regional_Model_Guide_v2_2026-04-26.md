...
산출물 경로: `/home/opc/oci-genai-guide-maintenance/runs/OCI_GenAI_Regional_Model_Guide_v2_2026-04-26.md`
...
- `2026-03-24` 기준, Oracle이 `xAI Grok 4.20` 및 `Grok 4.20 Multi-Agent`를 OCI Generative AI에 추가했다.
...
- dedicated retirement window 주의 모델:
  ...
  - Oracle 문서상 dedicated retirement date는 `No sooner than 2026-03-29`
...
| GPU shape 조회 | `oci --region ap-seoul-1 compute shape list --all -c <tenancy_ocid>` | 실패 | `timeout` 종료 코드 `124` |
...
| `compute shape list` | `iaas.ap-seoul-1.oraclecloud.com/20160918/shapes`로 GET 요청이 반복되지만 응답 완료 전에 `timeout` 종료 | Compute/Data Science shape 문서 기준 해석표로 대체 |
...
| 실제 생성 가능 여부 | 문서만으로 확정 불가 | 서비스 limit, host capacity, 예약 필요 여부를 추가 확인 |
...
| `meta.llama-4-scout-17b-16e-instruct` | 멀티모달 | 192k | 예(Chicago) | 불가 | 작은 footprint, agentic/coding |
| `meta.llama-4-maverick-17b-128e-instruct-fp8` | 멀티모달 | 512k | 예(Chicago) | 불가 | 긴 문맥, 코딩/추론 |
...
- xAI Grok 계열도 Oracle 문서상 온디맨드 전용이며 `external calls` 설명이 있다.
...
| `meta.llama-4-scout-17b-16e-instruct` | `LARGE_GENERIC_V2 x1` | Chicago만 온디맨드 | 불가 | 작은 footprint |
| `meta.llama-4-maverick-17b-128e-instruct-fp8` | `LARGE_GENERIC_2 x1` | Chicago만 온디맨드 | 불가 | 512k 컨텍스트 |
...
- 아래 표는 Oracle의 `Supported Models for Import` 계열 문서에 명시된 `Recommended Dedicated AI Cluster Unit Shape`를 우선 반영했다.
