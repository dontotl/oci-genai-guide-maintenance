...
- Oracle 공식 문서에 없는 리전별 고정 재고표는 `없음`으로 적었습니다.
- `관리형 기본 모델용 DAC unit`과 `imported model용 DAC unit`을 구분했습니다.
- `LARGE_COHERE_*`, `LARGE_GENERIC_*`, `SMALL_GENERIC_*` 같은 일부 DAC unit은 Oracle이 underlying hardware를 고객에게 숨기므로, GPU 종류와 GPU 메모리를 단정하지 않았습니다.
- `GPU 메모리 계산`은 오직 `A10/A100/H100/H200`처럼 이름에 GPU 종류와 개수가 드러나는 unit만 계산했습니다.
...
- 이번 실행에서는 `NotAuthorizedOrNotFound`보다 `endpoint timeout`이 먼저 확인되었습니다.
- 따라서 이 문서의 `IaaS/AQUA 리전별 재고표`는 `실제 테넌시 live inventory`가 아니라 `Oracle 문서 기준 해석표`입니다.
...
- 명령 자체는 유효합니다.
- 이번 환경에서는 두 명령 모두 `endpoint connection timeout`으로 자동 수집에 실패했습니다.
...
- 위 메모리 표는 Compute/Data Science 공식 shape 문서에 공개된 GPU 메모리만 썼습니다.
- `generic/cohere` 계열 DAC unit은 이 규칙으로 환산하지 않습니다.
...
- Gemini 계열은 Oracle 문서상 `external calls` 설명이 붙는 온디맨드 모델입니다.
- xAI Grok 계열도 Oracle 문서상 `external calls` 설명이 붙는 온디맨드 모델입니다.
- `meta.llama-4-*` 온디맨드는 현재 문서상 `US Midwest (Chicago)`에 한정되어 보입니다.
...
- `imported model을 배포할 수 있다`와 `OCI에서 fine-tuning을 제공한다`는 같은 뜻이 아닙니다.
...
- Oracle 공식 문서는 `imported model마다 권장 DAC가 모델별로 다르다`는 식으로 안내하지만, 모든 imported model을 하나로 묶은 `단일 정답표`는 제공하지 않습니다.
- 따라서 아래 표는 `Oracle이 공개한 GPU 메모리`만으로 만든 보수적 시작점입니다.
- 실제 선택은 `vendor model card의 VRAM 요구량`, `양자화 여부`, `컨텍스트 길이`, `동시성`, `LoRA adapter 유무`로 최종 확정해야 합니다.
- 이 절은 `imported model / custom deployment` 기준이며, 관리형 `gpt-oss` 전용 `OAI_*` unit은 별도 표로 봐야 합니다.
...
- 아주 작은 모델 검증용이면 `A10_X1` 또는 `A10_X2`부터 시작하는 편이 안전합니다.
