...
- `2026-03-24` 기준, Oracle release notes에 `xAI Grok 4.20`과 `xAI Grok 4.20 Multi-Agent` 추가가 반영되었다.
- `2026-03-04` 기준, 관리형 `OpenAI gpt-oss` 전용 DAC 가시성이 `UAE East (Dubai)`, `Saudi Arabia Central (Riyadh)`, `US West (Phoenix)`까지 확대되었다.
- `2026-02-26` 기준, `Cohere Embed 4` 온디맨드 지원 리전에 `US East (Ashburn)`, `Saudi Arabia Central (Riyadh)`가 추가되었다.
- `2026-01-21` 전후 기준, Oracle이 `Models by Region` / `Dedicated Cluster Shapes by Region` 페이지를 별도로 제공하기 시작했고, `Cohere Command A Vision`, `Cohere Command A Reasoning`, `xAI Grok 4.1 Fast`가 현재 문서 체계에 반영되었다.
- retired/deprecated 관점에서 신규 설계에서 먼저 제외할 모델군은 여전히 분명하다.
  ...
  - dedicated retirement window 주의: `Cohere Embed English Light 3`, `Cohere Embed Multilingual Light 3`는 Oracle 문서상 dedicated retirement date가 `No sooner than 2026-03-29`로 표시된다.
- 이번 문서 생성 시 OCI CLI 자동 조회는 성공하지 못했다.
  - `region-subscription list`: 실패
  - `compute shape list`: 실패
  - 보조 연결 확인용 `oci os ns get`: 실패
  - 실패 원인은 `권한 부족`이 아니라 `endpoint connection timeout`으로 관찰되었고, 따라서 `IaaS/AQUA 리전별 실제 GPU 재고`는 Oracle 문서 기준 해석표로 대체했다.
...
- `Imported model`: 사용자가 가져와 배포하는 커스텀/서드파티 모델

### 0-2. 문서 해석 원칙

- Oracle 공식 문서에 있는 사실만 확정적으로 썼다.
- Oracle 공식 문서에 없는 리전별 고정 재고표는 `없음`으로 적었다.
- `관리형 기본 모델용 DAC unit`과 `imported model용 DAC unit`을 구분했다.
- `LARGE_COHERE_*`, `LARGE_GENERIC_*`, `SMALL_GENERIC_*` 같은 일부 DAC unit은 Oracle이 underlying hardware를 고객에게 숨기므로, GPU 종류와 GPU 메모리를 단정하지 않았다.
- `GPU 메모리 계산`은 오직 `A10/A100/H100/H200`처럼 이름에 GPU 종류와 개수가 드러나는 unit만 계산했다.
...
| 조회 항목 | 실행 명령 | 상태 | 비고 |
|---|---|---|---|
| 구독 리전 조회 | `oci iam region-subscription list --all --output table` | 실패 | 설정 파일은 읽혔지만 API 응답이 타임아웃 |
| GPU shape 조회 | `oci --region ap-seoul-1 compute shape list --all -c <tenancy_ocid> ...` | 실패 | API 응답 타임아웃 |
| GPU shape 조회 | `oci --region us-ashburn-1 compute shape list --all -c <tenancy_ocid> ...` | 실패 | API 응답 타임아웃 |
| 보조 연결 확인 | `oci os ns get --output table` | 실패 | `objectstorage.ap-seoul-1.oraclecloud.com` 연결 타임아웃 |

### 1-2. 실패 이유

| 항목 | 관찰 내용 | 문서 반영 방식 |
|---|---|---|
| OCI CLI 설정 | `~/.oci/config`에서 user / tenancy / region 확인됨 | 명령 자체는 유효한 환경으로 판단 |
| `region-subscription list` | `RequestException: The connection to endpoint timed out.` | Oracle 리전 문서 기준 표로 대체 |
| `compute shape list` | `RequestException: The connection to endpoint timed out.` | Compute/Data Science shape 문서 + 해석표로 대체 |
| `oci os ns get` | `https://objectstorage.ap-seoul-1.oraclecloud.com` 연결 타임아웃 | 이번 실패는 인증 이전 네트워크 또는 endpoint reachability 문제로 해석 |

실무 메모:

- 이번 실행에서는 `NotAuthorizedOrNotFound`보다 `endpoint timeout`이 먼저 확인되었다.
- 따라서 이 문서의 `IaaS/AQUA 리전별 재고표`는 `실제 테넌시 live inventory`가 아니라 `Oracle 문서 기준 해석표`다.
...
| SOV | EU Sovereign Central (Frankfurt) | 지원 | 지원(모델별) | Oracle 문서상 명시 없음 | AQUA sovereign 지원 여부는 이번 기준 미확인 |
...
- 명령 자체는 유효하다.
- 이번 환경에서는 두 명령 모두 `endpoint connection timeout`으로 자동 수집에 실패했다.
...
- 위 메모리 표는 Compute/Data Science 공식 shape 문서에 공개된 GPU 메모리만 썼다.
- `generic/cohere` 계열 DAC unit은 이 규칙으로 환산하지 않는다.
...
| `xAI Grok 4.1 Fast 계열` | fast reasoning | 2M | 예 | 불가 | 초장문 문맥 + 빠른 응답 |
| `xAI Grok 4.20 계열` | frontier reasoning | 2M | 예 | 불가 | 복합 reasoning / research 성격 질의 |
...
- Gemini 계열은 Oracle 문서상 `external calls` 설명이 붙는 온디맨드 모델이다.
- xAI Grok 계열도 Oracle 문서상 `external calls` 설명이 붙는 온디맨드 모델이다.
- `meta.llama-4-*` 온디맨드는 현재 문서상 `US Midwest (Chicago)`에 한정되어 보인다.
...
| `meta.llama-3.3-70b-instruct` | `LARGE_GENERIC x1` | on-demand + custom model 둘 다 자주 참조 | 가능 | 현재 active fine-tuning 기반 모델 |
...
|---|---|---|---|
| `meta.llama-3.3-70b-instruct` | 가능 | LoRA | Oracle fine-tuning 문서 기준 |
| `cohere.command-r-08-2024` | 가능 | T-Few, LoRA | Oracle fine-tuning 문서 기준 |
...
- `imported model을 배포할 수 있다`와 `OCI에서 fine-tuning을 제공한다`는 같은 뜻이 아니다.
...
- Oracle 공식 문서는 `imported model마다 권장 DAC가 모델별로 다르다`는 식으로 안내하지만, 모든 imported model을 하나로 묶은 `단일 정답표`는 제공하지 않는다.
- 따라서 아래 표는 `Oracle이 공개한 GPU 메모리`만으로 만든 보수적 시작점이다.
- 실제 선택은 `vendor model card의 VRAM 요구량`, `양자화 여부`, `컨텍스트 길이`, `동시성`, `LoRA adapter 유무`로 최종 확정해야 한다.
- 이 절은 `imported model / custom deployment` 기준이며, 관리형 `gpt-oss` 전용 `OAI_*` unit은 별도 표로 봐야 한다.
...
- 아주 작은 모델 검증용이면 `A10_X1` 또는 `A10_X2`부터 시작하는 편이 안전하다.
...
| `cohere.command-a-reasoning` | 긴 문서와 복합 reasoning 전용 DAC 워크로드 |
...
| `xAI Grok 4.1 Fast 계열` | 초장문 문맥과 빠른 응답 |
| `xAI Grok 4.20 계열` | 고난도 reasoning / research 성격 응답 |
