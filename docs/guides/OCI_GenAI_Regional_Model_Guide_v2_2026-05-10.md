...
- 반대로 `timeout 15s` 안에서 endpoint 요청이 반복되다가 종료되는 패턴은 확인했습니다.
...
- 표 폭을 줄이기 위해 범용/멀티모달/임베딩·재정렬로 분리했습니다.
...
- `cohere.rerank.v3-5`는 dedicated only라서 `온디맨드 핵심 모델 표`에서는 재정렬 기준 참고 항목으로만 넣었습니다.
