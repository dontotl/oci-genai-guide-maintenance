...
향후 cron으로 생성되는 최신 가이드에도 아래 표현을 유지합니다.

- 상단 링크는 `github 페이지 웹사이트 :`, `Private endpoint 가이드 :` 표현을 사용
- 기준 데이터는 `기준 데이터 : <날짜>, 출처는 OCI Docs, OCI CLI 조회` 형식 사용
- 문서 성격은 `문서 성격 : OCI Gen AI 구성 가이드를 위한 문서입니다.` 사용
- 고객용 본문에서는 `catalog JSON`, `AI catalog 공개 JSON`, `utility`, `Codex 샌드박스`, `고객용 probe summary 기준` 표현을 피함
- 고객용 본문에서는 `OCI CLI를 통한 리소스 조회 결과`, `보조/안전 모델` 표현을 사용
- 11번 `리전 및 가용성 확인`에는 전체 리전 matrix를 별도 11-3 섹션으로 넣지 않음
